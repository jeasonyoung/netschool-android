...
public class FreeExperienceFragmentByPackages extends Fragment {
    ...
    public FreeExperienceFragmentByPackages(){
        this.packageClasses = new ArrayList<>();
     ...
     * @return 当前对象。
     */
    public FreeExperienceFragmentByPackages setExamId(String examId) {
        this.examId = examId;
        return this;
    }
     ...
     * @return 当前对象。
     */
    public FreeExperienceFragmentByPackages setSearch(Search search) {
        this.search = search;
        if(search != null) {
            this.search = search;
	...
	private void gotoLessonFragment(PackageClass data){
		...
		getActivity().getSupportFragmentManager()
						.beginTransaction()
						.addToBackStack(null)
						.replace(R.id.fragment_container, new FreeExperienceFragmentByLesson()
								.setSearch(this.search)
                                .setClassId(data.getId()))
						.commit();
	}
	...
	private class AsyncLoadData  extends AsyncTask<Void, Void, List<PackageClass>>{
		...
		protected List<PackageClass> doInBackground(Void... params) {
			try{
				...
				//请求数据
				final JSONCallback<PackageClass[]> callback = new APIUtils.CallbackJSON<>(getActivity(), PackageClass[].class)
	...
	private class PackageAdapter extends BaseExpandableListAdapter{
		...
		public int getChildrenCount(int groupPosition) {
			this.childArrays = new SparseArray<>();
			if(this.list != null && this.groups != null && this.groups.size() > groupPosition){
				...
				if(group != null && StringUtils.isNotBlank(group.getId())){
					final List<PackageClass> childs = new ArrayList<>();
					for(PackageClass data : this.list){
						if(data != null && StringUtils.equalsIgnoreCase(data.getPid(), group.getId())){
							childs.add(data);
						}
					}
					this.childArrays.put(groupPosition, childs.toArray(new PackageClass[childs.size()]));
				}
			}
