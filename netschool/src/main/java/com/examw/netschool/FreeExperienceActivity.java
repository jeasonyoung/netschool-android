...
public class FreeExperienceActivity extends FragmentActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(tvTopTitle != null) tvTopTitle.setText(R.string.free_experience_title);
		
		//搜索
		this.search = new Search(this);
		//课程中心
		final View btnCenter = this.findViewById(R.id.btn_free_experience);
		if(btnCenter != null) btnCenter.setOnClickListener(this);
		//我的课程
		final View btnMy = this.findViewById(R.id.btn_my_course);
		if(btnMy != null) btnMy.setOnClickListener(this);
		//播放记录
		final View btnRecord = this.findViewById(R.id.btn_play_record);
		if(btnRecord != null) btnRecord.setOnClickListener(this);
	}
	/*
	 * 重载启动。
	...
	private void loadFirstFragment(){
		...
		.addToBackStack(null)
		.add(R.id.fragment_container, new FreeExperienceFragmentByExams().setSearch(this.search))
		.commit();
	}
	/*
	 * 点击事件处理。
