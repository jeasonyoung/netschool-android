...
public class VideoPlayActivity extends BaseActivity {
	...
	private void createVideoView(){
		...
		//显示加载进度
		if(this.videoLoadingView != null) this.videoLoadingView.setVisibility(View.VISIBLE);
	}
	...
	private void createVolumnControlBar(LayoutInflater inflater){
		...
		if(audioManager != null){
			...
			//显示音量百分数
			this.tvVolumnSize.setText(String.format("%d%%",current * 100 / volumnMax));
		}
		//设置音量控制器
		...
		this.volumnBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				...
				if(tvVolumnSize != null) tvVolumnSize.setText(String.format("%d%%",vol * 100 / seekBar.getMax()));
			}
            ...
            this.videoBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
                ...
                public void onStopTrackingTouch(SeekBar seekBar) {
                    ...
                    if (playVideoView != null) {
                        playVideoView.seekTo(this.pos);
                    }
                    //更改播放时间显示
                    if (tvCurrentTime != null) {
                        tvCurrentTime.setText(Utils.getTime(this.pos / 1000));
                    }
                }
	...
	private OnCompletionListener onCompletionListener = new OnCompletionListener(){
		...
		public void onCompletion(MediaPlayer mp) {
			...
			if(videoBar != null) videoBar.setProgress(0);
			//播放时间
			if(tvCurrentTime != null) tvCurrentTime.setText(String.format("%s","00:00"));
			//播放器
			if(playVideoView != null){
		...
		AppContext.pools_fixed.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					//初始化
					final PlayRecordDao playRecordDao = new PlayRecordDao();
					playRecordDao.updatePlayTime(recordId, pos);
				} catch (Exception e) {
					...
					if(newVolume >= 0){
						...
						if(this.topBar != null && this.topBar.isShowing()){
							...
							if(this.tvVolumnSize != null && newVolume < this.volumnMax){
								this.tvVolumnSize.setText(String.format("%d%%",newVolume * 100 / this.volumnMax));
							}
						}
					}
				}
