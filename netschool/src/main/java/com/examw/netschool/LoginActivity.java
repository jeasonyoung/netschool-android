...
public class LoginActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if(btnOnlineLogin != null) btnOnlineLogin.setOnClickListener(this);
		//离线登录
		final View btnOfflineLogin = this.findViewById(R.id.btn_login_offline);
		if(btnOfflineLogin != null) btnOfflineLogin.setOnClickListener(this);
	}
	//用户名编辑监听事件处理。
				...
				if(StringUtils.isNotBlank(username) && txtPassword != null){
					...
					if(userShared.contains(username)){
						...
						if(StringUtils.isNotBlank(enpwd)){
							//设置密码
							txtPassword.setText(Hex.base64DecodeToString(enpwd));
		...
		new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... params) {
				try{
					...
					//参数处理
					final String pwd_source = _username + Hex.md5Hex(_password);
					Log.d(TAG, "加密密码原文:" +pwd_source);
					final Map<String, Object> parameters = new HashMap<>();
					parameters.put("username", _username);
					parameters.put("pwd", Hex.md5Hex(pwd_source));
					parameters.put("terminal", Constant.TERMINAL);
					//验证用户
					final JSONCallback<LoginResult> callback = new APIUtils.CallbackJSON<>(null, LoginResult.class)
		...
		new AsyncTask<String, Void, String>() {
			...
			protected String doInBackground(String... params) {
				try{
					...
					if(!StringUtils.equalsIgnoreCase(enpwd, Hex.md5Hex(_username + _password))){
						Log.d(TAG, "本地登录密码验证失败!" + _password);
						return getResources().getText(R.string.login_fail_local_pwd).toString();
					}
					//获取用户ID
					_userId = userPwdShared.getString(Constant.PREFERENCES_CONFIG_USERPWD_USERID + Hex.md5Hex(_username), "");
				...
				try{
					...
					if(userSharedPreferences != null){
						userSharedPreferences.edit()
                                .putString(userName, Hex.base64EncodeToString(password))
                                .apply();
					}
				}catch(Exception e){
            ...
            public void run() {
                try {
                    ...
                    if (userPwdSharedPreferences != null) {
                        //用户密码加密存储
                        final String user_pwd = Hex.md5Hex(username + password);
                        //记住用户密码
                        userPwdSharedPreferences.edit()
                                .putString(Constant.PREFERENCES_CONFIG_USERPWD_USERID + Hex.md5Hex(username), userId)
			...
			if(userShared != null && this.txtUsername != null){
				...
				if(users != null && users.size() > 0){
                    for(Map.Entry<String,?> entry : users.entrySet()){
                        ...
                        //密码
                        final String enpwd = userShared.getString(entry.getKey(), null);
                        if(StringUtils.isNotBlank(enpwd)){
                            this.txtPassword.setText(Hex.base64DecodeToString(enpwd));
